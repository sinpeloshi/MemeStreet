...
export default function MarketPage() {
  ...
  const bet = async () => {
    ...
    if (!pos) { setMsg({ t: 'err', v: 'Select YES or NO first' }); return }
    setBetting(true); setMsg({ t: '', v: '' })
    const res = await fetch(`/api/markets/${id}/bet`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ position: pos === 'yes', amount }),
    })
    const data = await res.json()
    if (!res.ok) { setMsg({ t: 'err', v: data.error }); setBetting(false); return }
    setMsg({ t: 'ok', v: `BET PLACED — ${amount} ◈ on ${pos.toUpperCase()}` })
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', padding: '100px 20px' }}>
      <div style={{ fontFamily: 'Bebas Neue', fontSize: '32px', letterSpacing: '4px', color: 'var(--lime)' }}>
        LOADING MARKET...
      </div>
    </div>
  )
  ...
  if (!market || market.error) return (
    <div style={{ textAlign: 'center', padding: '80px' }}>
      <div style={{ fontFamily: 'Bebas Neue', fontSize: '48px', color: 'var(--muted2)', letterSpacing: '3px' }}>404 — MEME NOT FOUND</div>
    </div>
  )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                {isOpen ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--lime-dim)', border: '1px solid rgba(181,255,0,0.3)', borderRadius: '5px', padding: '3px 10px' }}>
                    ...
                    <span style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', fontWeight: 700, color: 'var(--lime)', letterSpacing: '1.5px' }}>LIVE</span>
                  </div>
                ) : (
              ...
              <h1 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '6px', letterSpacing: '-0.3px' }}>
                {market.meme.title}
              </h1>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
            <div className="section-head" style={{ color: 'var(--muted)', marginBottom: '20px' }}>
              MARKET STATS
            </div>
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '10px' }}>
                <div>
                  <div style={{ fontFamily: 'Bebas Neue', fontSize: '48px', color: 'var(--lime)', letterSpacing: '2px', lineHeight: 1, textShadow: '0 0 30px rgba(181,255,0,0.25)' }}>
                    YES {yesP}%
                  </div>
                  <div style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: 'var(--muted)', letterSpacing: '1px', marginTop: '2px' }}>
                    ◈ {market.totalYes.toLocaleString()} in pool
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontFamily: 'Bebas Neue', fontSize: '48px', color: 'var(--red)', letterSpacing: '2px', lineHeight: 1 }}>
                    {noP}% NO
                  </div>
                  <div style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: 'var(--muted)', letterSpacing: '1px', marginTop: '2px', textAlign: 'right' }}>
                    ◈ {market.totalNo.toLocaleString()} in pool
                  </div>
                </div>
              </div>
              <div style={{ height: '8px', background: 'var(--border2)', borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{
                  ...
                  boxShadow: '0 0 12px rgba(181,255,0,0.4)',
                }} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
              {[
                ...
                { l: 'PLATFORM', v: market.platform.toUpperCase(), c: 'var(--muted)' },
                { l: 'TIME', v: timeLeft(market.deadline), c: isOpen ? 'var(--lime)' : 'var(--red)' },
              ].map(s => (
            ...
            <div style={{ marginTop: '12px', fontFamily: 'JetBrains Mono', fontSize: '11px', color: 'var(--muted2)' }}>
              🎯 Threshold: {market.threshold.toLocaleString()} interactions on {market.platform}
            </div>
          </div>
          ...
          {market.bets.length > 0 && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
              <div className="section-head" style={{ color: 'var(--muted)', marginBottom: '14px' }}>
                RECENT ACTIVITY
              </div>
                ...
                <div key={b.id} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{
                      ...
                      background: b.position ? 'var(--lime-dim)' : 'var(--red-dim)',
                      color: b.position ? 'var(--lime)' : 'var(--red)',
                      border: `1px solid ${b.position ? 'rgba(181,255,0,0.3)' : 'rgba(255,45,85,0.3)'}`,
                      padding: '2px 8px', borderRadius: '4px', letterSpacing: '1px',
                    }}>
                      {b.position ? 'YES' : 'NO'}
                    </span>
                    <span style={{ fontFamily: 'JetBrains Mono', fontSize: '12px', color: '#888' }}>
                      @{b.user.username}
                    </span>
                  </div>
                  <span style={{ fontFamily: 'JetBrains Mono', fontSize: '12px', fontWeight: 700, color: 'var(--gold)' }}>
              ...
              <div style={{ padding: '40px 24px', textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'Bebas Neue', fontSize: '28px', letterSpacing: '2px', color: '#fff', marginBottom: '8px' }}>
                  {market.status === 'RESOLVED'
                    ? `${market.result ? 'YES' : 'NO'} WON`
                    : 'MARKET CLOSED'}
                </div>
                <p style={{ fontFamily: 'JetBrains Mono', fontSize: '11px', color: 'var(--muted)', letterSpacing: '0.5px' }}>
                  {market.status === 'RESOLVED' ? 'Winners have been paid' : 'Awaiting resolution'}
                </p>
              </div>
            ) : (
              <>
                ...
                <div style={{
                  ...
                  <div style={{ fontFamily: 'JetBrains Mono', fontSize: '11px', fontWeight: 700, letterSpacing: '2px', color: 'var(--lime)' }}>
                    ⚡ PLACE BET
                  </div>
                  {user && (
                    <div style={{ fontFamily: 'JetBrains Mono', fontSize: '11px', color: 'var(--gold)' }}>
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  {msg.v && (
                    <div style={{
                      background: msg.t === 'ok' ? 'var(--lime-dim)' : 'var(--red-dim)',
                      border: `1px solid ${msg.t === 'ok' ? 'rgba(181,255,0,0.3)' : 'rgba(255,45,85,0.3)'}`,
                      color: msg.t === 'ok' ? 'var(--lime)' : 'var(--red)',
                      padding: '10px 14px',
                      borderRadius: '8px',
                      fontSize: '12px',
                      fontFamily: 'JetBrains Mono',
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '18px' }}>
                    {[
                      { k: 'yes', label: 'YES', pct: yesP, c: 'var(--lime)', bg: 'var(--lime-dim)', border: 'rgba(181,255,0,0.5)' },
                      { k: 'no',  label: 'NO',  pct: noP,  c: 'var(--red)',  bg: 'var(--red-dim)',  border: 'rgba(255,45,85,0.5)' },
                    ].map(o => (
                      <button key={o.k} onClick={() => setPos(o.k)} style={{
                        background: pos === o.k ? o.bg : 'var(--surface2)',
                        border: `1px solid ${pos === o.k ? o.border : 'var(--border2)'}`,
                        borderRadius: '10px',
                        padding: '18px 10px',
                        cursor: 'crosshair',
                        transition: 'all 0.15s',
                        boxShadow: pos === o.k ? `0 0 20px ${o.bg}` : 'none',
                      }}>
                        <div style={{
                          fontFamily: 'Bebas Neue',
                          fontSize: '32px',
                          letterSpacing: '2px',
                          color: pos === o.k ? o.c : 'var(--muted)',
                          lineHeight: 1,
                        }}>
                          {o.label}
                        </div>
                  ...
                  <div style={{ marginBottom: '14px' }}>
                    <label className="field-label">CREDITS TO BET</label>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginTop: '8px' }}>
                      {[50, 100, 250, 500].map(a => (
                        <button key={a} onClick={() => setAmount(a)} style={{
                          background: amount === a ? 'var(--border2)' : 'var(--surface2)',
                          border: `1px solid ${amount === a ? 'var(--border2)' : 'var(--border)'}`,
                          color: amount === a ? '#fff' : 'var(--muted)',
                          fontFamily: 'JetBrains Mono',
                          fontSize: '11px',
                          fontWeight: 600,
                          padding: '7px',
                  ...
                  {pos && (
                    <div style={{
                      ...
                      <div style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', color: 'var(--muted)', letterSpacing: '1.5px', marginBottom: '4px' }}>
                      ...
                      <div style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', color: 'var(--muted2)', marginTop: '2px' }}>
                        {payout > amount
                          ? `+${(((payout - amount) / amount) * 100).toFixed(0)}% ROI`
                          : 'LOW ODDS — CONSIDER OTHER SIDE'}
                      </div>
                    </div>
                  )}

                  {/* Submit */}
                  <button onClick={bet} disabled={betting || !pos} className="btn-lime"
                    style={{ width: '100%', padding: '14px', borderRadius: '8px', fontSize: '14px', letterSpacing: '0.5px', opacity: (!pos || betting) ? 0.3 : 1 }}>
                    {betting ? 'EXECUTING...' : user ? `BET ${amount} ◈ ON ${pos ? pos.toUpperCase() : '?'}` : 'LOGIN TO BET'}
                  </button>
